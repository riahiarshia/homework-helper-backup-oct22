...
router.post('/session/start-text', async (req, res) => {
    try {
        ...
        // Analyze problem using OpenAI (same code path as iOS image endpoint)
        const analysisResult = await openaiService.analyzeHomework({
            imageData: null, // No image for text testing
            ...
            deviceId: deviceId,
            metadata: {
                gradeLevel: userGradeLevel || '6th grade',
                sessionType: 'step-by-step-tutoring',
                hasImage: false,
                hasTeacherMethod: false,
                testMode: true
            }
        });
