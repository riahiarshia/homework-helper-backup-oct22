...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 32) {
                        ...
                        if selectedImage != nil && dataManager.pendingNavigationProblemId == nil {
                            imagePreview
                        }
            ...
            .navigationDestination(isPresented: Binding(
                get: { dataManager.pendingNavigationProblemId != nil },
                set: { if !$0 { dataManager.pendingNavigationProblemId = nil } }
            )) {
                destinationView
            }
    ...
    private var destinationView: some View {
        Group {
            if let problemId = dataManager.pendingNavigationProblemId {
    ...
    private func analyzeProblem() async {
        ...
        do {
            ...
            // Add the first step from the new step-by-step format
            print("🔍 DEBUG HomeView: Adding first step to DataManager")
            let firstStep = GuidanceStep(
                problemId: problem.id,
                stepNumber: 1,
                question: analysis.currentStep.question,
                explanation: analysis.currentStep.explanation,
                options: analysis.currentStep.options,
                correctAnswer: analysis.currentStep.correctAnswer
            )
            dataManager.addStep(firstStep, for: problem.id)
            
            print("🔍 DEBUG HomeView: First step added successfully")
            print("✅ DEBUG HomeView: Problem ready for step-by-step guidance")
            ...
            await MainActor.run {
                ...
                dataManager.pendingNavigationProblemId = problem.id
                selectedImage = nil
            }
    ...
    private func performImageAnalysis(problemImage: UIImage, teacherMethodImage: UIImage?) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            do {
                ...
                // Start a tutoring session instead of just analyzing
                let session = try await backendService.startTutoringSessionWithImage(
                    imageData: imageData,
                    userGradeLevel: userGradeLevel,
                    teacherMethodImageData: teacherMethodImageData
                )
                
                logger.critical("🚨 CRITICAL DEBUG: Tutoring session started successfully!")
                logger.critical("🚨 CRITICAL DEBUG: Session - Subject: \(session.subject ?? "Unknown"), Difficulty: \(session.difficulty ?? "Unknown"), Total Steps: \(session.totalSteps)")
                
                // Capture teacherMethodImageData before MainActor to avoid concurrency issues
                let capturedMethodData = teacherMethodImageData
                
                await MainActor.run {
                    // Create problem from session
                    var problem = HomeworkProblem(
                        id: UUID(),
                    ...
                    // Add ALL steps from the tutoring session
                    if let steps = session.steps {
                        for (index, stepData) in steps.enumerated() {
                            guard let unwrappedStep = stepData else { continue }
                            let step = GuidanceStep(
                                problemId: problem.id,
                                stepNumber: index + 1,
                                question: unwrappedStep.question,
                                explanation: unwrappedStep.explanation,
                                options: unwrappedStep.options,
                                correctAnswer: unwrappedStep.correctAnswer
                            )
                            dataManager.addStep(step, for: problem.id)
                        }
                        logger.critical("✅ Added \(steps.count) steps to DataManager")
                    } else {
                        ...
                        dataManager.addStep(firstStep, for: problem.id)
                        logger.critical("⚠️ No steps from backend, added fallback step")
                    }
                    
                    logger.critical("🚨 CRITICAL DEBUG: Setting pendingNavigationProblemId = \(problem.id)")
                    dataManager.pendingNavigationProblemId = problem.id
                    logger.critical("🚨 CRITICAL DEBUG: pendingNavigationProblemId is now: \(String(describing: dataManager.pendingNavigationProblemId))")
                    isProcessing = false
                }
    ...
    private func analyzeImageIgnoringQuality() async {
        ...
        print("🔍 DEBUG HomeView: Image data prepared for tutoring session")
        print("   Image size: \(image.size)")
        print("   Image data size: \(imageData.count) bytes")
        print("   Image data first 10 bytes: \(imageData.prefix(10).map { String(format: "%02x", $0) }.joined(separator: " "))")
        ...
        do {
            ...
            await MainActor.run {
                ...
                if let steps = session.steps {
                    ...
                    print("✅ Added \(steps.count) steps to DataManager")
                } else {
                    // Fallback if no steps provided
                    let firstStep = GuidanceStep(
                        problemId: problem.id,
                    ...
                    dataManager.addStep(firstStep, for: problem.id)
                    print("⚠️ No steps from backend, added fallback step")
                }
