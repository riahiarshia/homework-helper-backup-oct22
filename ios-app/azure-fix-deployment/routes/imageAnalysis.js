...
router.post('/validate-image', upload.single('image'), async (req, res) => {
    try {
        ...
        loggingService.writeLog(loggingService.formatLogEntry('API_ENDPOINT_ACCESS', {
            endpoint: '/api/validate-image',
            method: 'POST',
            userId,
            deviceId,
            metadata: {
    ...
    try {
        ...
        loggingService.writeLog(loggingService.formatLogEntry('API_ENDPOINT_ACCESS', {
            ...
            metadata: {
                ...
                problemText: problemText ? problemText.substring(0, 200) : null
            }
        }));
...
router.post('/hint', async (req, res) => {
    try {
        ...
        loggingService.writeLog(loggingService.formatLogEntry('API_ENDPOINT_ACCESS', {
            ...
            metadata: {
                gradeLevel: userGradeLevel,
                stepQuestion: step ? step.question : null,
                problemContext: problemContext ? problemContext.substring(0, 200) : null
            }
        }));
