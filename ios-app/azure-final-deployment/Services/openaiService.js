...
class OpenAIService {
  ...
  async validateImageQuality({ imageData, apiKey, userId, deviceId, metadata = {} }) {
    ...
    try {
      // Log OpenAI request
      loggingService.logOpenAIRequest('validate_image', {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId }
      });
      ...
      if (response.data && response.data.choices && response.data.choices[0]) {
        ...
        // Log OpenAI response
        loggingService.logOpenAIResponse('validate_image', response.data, {
          userId,
          deviceId,
          metadata
        });
        ...
        loggingService.logImageValidation(userId, deviceId, {
          ...result,
          metadata
        });
      ...
      // Log OpenAI error
      loggingService.logOpenAIError('validate_image', error, {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId }
      });
  ...
  async analyzeHomework({ imageData, problemText, teacherMethodImageData, userGradeLevel, apiKey, userId, deviceId, metadata = {} }) {
    ...
    try {
      // Log homework submission context
      loggingService.logHomeworkSubmission(userId, deviceId, {
        problemId: metadata.problemId || 'unknown',
        subject: 'unknown',
        difficulty: 'unknown',
      ...
      // Log OpenAI request
      loggingService.logOpenAIRequest('analyze_homework', {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId, userGradeLevel, hasImage: !!imageData, hasTeacherMethod: !!teacherMethodImageData }
      });
      ...
      if (response.data && response.data.choices && response.data.choices[0]) {
        ...
        // Log OpenAI response
        loggingService.logOpenAIResponse('analyze_homework', response.data, {
          userId,
          deviceId,
          metadata
        });

        // Log homework analysis result
        loggingService.logHomeworkAnalysis(userId, deviceId, result);
      ...
      // Log OpenAI error
      loggingService.logOpenAIError('analyze_homework', error, {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId }
      });
...
Provide a helpful, age-appropriate hint that is DIRECTLY RELATED to the problem context and question above.`;
    ...
    try {
      // Log OpenAI request
      loggingService.logOpenAIRequest('generate_hint', {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId, userGradeLevel }
      });
      ...
      if (response.data && response.data.choices && response.data.choices[0]) {
        ...
        // Log OpenAI response
        loggingService.logOpenAIResponse('generate_hint', response.data, {
          userId,
          deviceId,
          metadata
        });

        // Log hint generation result
        loggingService.logHintGeneration(userId, deviceId, {
          step,
          problemContext,
          userGradeLevel,
          hint: content,
      ...
      // Log OpenAI error
      loggingService.logOpenAIError('generate_hint', error, {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId }
      });
  ...
  async verifyAnswer({ answer, step, problemContext, userGradeLevel, apiKey, userId, deviceId, metadata = {} }) {
    ...
    try {
      // Log OpenAI request
      loggingService.logOpenAIRequest('verify_answer', {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId, userGradeLevel }
      });
      ...
      if (response.data && response.data.choices && response.data.choices[0]) {
        ...
        // Log OpenAI response
        loggingService.logOpenAIResponse('verify_answer', response.data, {
          userId,
          deviceId,
          metadata
        });

        // Log answer verification result
        loggingService.logAnswerVerification(userId, deviceId, {
          answer,
          step,
          problemContext,
          userGradeLevel,
      ...
      // Log OpenAI error
      loggingService.logOpenAIError('verify_answer', error, {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId }
      });
...
- High School (9-12): Use appropriate mathematical notation and abstract concepts`;
    ...
    try {
      // Log OpenAI request
      loggingService.logOpenAIRequest('chat_response', {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId, userGradeLevel }
      });
      ...
      if (response.data && response.data.choices && response.data.choices[0]) {
        ...
        // Log OpenAI response
        loggingService.logOpenAIResponse('chat_response', response.data, {
          userId,
          deviceId,
          metadata
        });

        // Log chat response result
        loggingService.logChatResponse(userId, deviceId, {
          messages,
          problemContext,
          userGradeLevel,
          response: content,
      ...
      // Log OpenAI error
      loggingService.logOpenAIError('chat_response', error, {
        ...requestBody,
        metadata: { ...metadata, userId, deviceId }
      });
